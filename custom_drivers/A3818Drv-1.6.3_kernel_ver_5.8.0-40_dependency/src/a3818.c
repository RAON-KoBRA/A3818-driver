/*
		...
		Feb          2021 : Maual Config Ver for the Linux Kernel 'Ver 5.8.0-40 - low latency' dependence
				     this version has a backward compitability. (it would fix kernel issue from 5.6 or higer)    
        ----------------------------------------------------------------------
*/
...
#include <linux/pci.h>
#if LINUX_VERSION_CODE >= VERSION(5,4,0)
#include <linux/vmalloc.h>
#elif LINUX_VERSION_CODE >= VERSION(2,6,26)
#include <linux/vmalloc.h>
...
//#include </usr/src/linux-headers-5.8.0-40-lowlatency/include/linux/proc_fs.h>
#endif
...
#if LINUX_VERSION_CODE >= VERSION(5,3,0)
	#define a3818_mmiowb()
#else
	#if LINUX_VERSION_CODE >= VERSION(2,6,10)
		#define a3818_mmiowb() 	mmiowb()
	#else
		#define a3818_mmiowb()
	#endif
#endif
...
#elif LINUX_VERSION_CODE >= VERSION(3,10,0)
...
#else
static struct proc_ops a3818_fops =
{
        proc_ioctl:         a3818_ioctl_unlocked,
        proc_open:                           a3818_open,
        proc_release:                        a3818_release
};
...
*/
  ...
  if( index > 0 ) {
      ...
      a3818_register_proc();
      a3818_major = register_chrdev(0, "a3818", (const struct file_operations *)&a3818_fops );
      if( a3818_major < 0 ) {
